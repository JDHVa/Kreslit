...
function nullG(){
    return{pinch:false, peace:false, three:false, fist:false, thumb:false, palm:false};
}
...
const CMD_INFO = {
    ...
    cmd_palm:{e:"",t:"Right Pinch or click → clean all"},
...
function executeCmd(){
    switch(S.leftState){
        ...
        case 'cmd_palm':  S.strokes=[]; S.cur=null; S.floats=[]; break;
...
function updateFSM(now){
    ...
    if(S.leftState === 'idle') {
        ...
        else if(Lg.peace)                      { S.leftState='cmd_peace'; S.peaceOrigin=lm2c(Lh.lms[9]); S.peaceHov=null; }
        ...
        else if(Lg.palm  && now-S.gestCD>CD)   { S.leftState='cmd_palm';  S.gestCD=now; }
...
function drawWheelIfNeeded(){
    ...
    cx.save();
    cx.beginPath();
    cx.arc(ox,oy,WHEEL_R+44,0,Math.PI*2);
    ...
    for(let i=0; i<n; i++){
        ...
        if(hov){ cx.strokeStyle="#fff"; cx.lineWidth=2.5; cx.stroke();
...
function drawCursors(){
    ...
    if(S.R.present && S.R.ip){
        ...
        if(S.R.pinching){
            ...
            cx.setLineDash([]);
            cx.beginPath();
            cx.arc(pos.x,pos.y, 3,0,Math.PI*2);
            cx.fillStyle = c;
            cx.fill();
        }
        cx.restore(); 
...
const STATE_TEXT = {
  idle:'Waiting for a gesture', draw:' Drawing Mode', cmd_peace:' Choose a color',
...
function updateUI(){
    ...
    if(S.R.present){ dotR.classList.add("on"); stxtR.textContent=S.R.pinching?"🤌 Pinch":"Detected";
        ssubR.textContent=S.leftState==='draw'?"Draw with your index":"Pinch to confirm"; }
...
async function analyze(){
    ...
    try{
        ...
        if(p.texto){ aiOCR.style.display="block"; 
...
clearBtn.addEventListener("click",()=>{ S.strokes=[]; S.cur=null; S.floats=[]; });
